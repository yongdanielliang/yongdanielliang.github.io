...
Tree24.prototype.getChildNode = function(e, node) {
    if (node.child.length == 0)
        return null; // node is a leaf

    var i = this.locate(e, node); // Locate the insertion point for e
...
Tree24.prototype.insert1 = function(e, rightChildOfe, u) {
    ...
    for (var i = path.length - 1; i >= 0; i--) {
        if (u.elements.length < 3) { // u is a 2-node or 3-node
            ...
            var median = this.split(e, rightChildOfe, u, v); // Split u

            if (u == this.root) {
                this.root = new Tree24Node(median); // New root
                this.root.child.push(u); // u is the left child of median
                this.root.child.push(v); // v is the right child of median
                break; // No further insertion to u's parent needed
            }
            else {
...
Tree24.prototype.insert23 = function(e, rightChildOfe, node) {
    ...
    node.elements.splice(i, 0, e); // Insert the element into the node
    if (rightChildOfe != null)
        node.child.splice(i + 1, 0, rightChildOfe); // Insert the child link
}

/** Split a 4-node u into u and v and insert e to u or v */
Tree24.prototype.split = function(e, rightChildOfe, u, v) {
    // Move the last element in node u to node v
    v.elements.push(u.elements.splice(2, 1)[0]);
    var median = u.elements.splice(1, 1)[0];
    // Split children for a non-leaf node
    // Move the last two children in node u to node v
    if (u.child.length > 0) {
        v.child.push(u.child.splice(2, 1)[0]);
        v.child.push(u.child.splice(2, 1)[0]);
    }
...
// Locate the rightmost node in the left subtree of the node 
        ...
        // Get the path that leads to e from the root
        var path = this.path(rightmostElement);
        // Replace the deleted element with the rightmost element
        node.elements[index] = current.elements.splice(current.elements.length - 1, 1)[0];
        this.validate(rightmostElement, current, path); // Check underflow
    }
}
...
Tree24.prototype.validate = function(e, u, path) {
    for (var i = path.length - 1; u.elements.length == 0; i--) {
        ...
        if (k > 0 && parentOfu.child[k - 1].elements.length > 1) {
            this.leftSiblingTransfer(k, u, parentOfu);
        }
        else if (k + 1 < parentOfu.child.length &&
                parentOfu.child[k + 1].elements.length > 1) {
            this.rightSiblingTransfer(k, u, parentOfu);
        }
            ...
            this.leftSiblingFusion(k, leftNode, u, parentOfu);
...
// Move an element from the parent to u
    u.elements.splice(0, 0, parentOfu.elements[k - 1]);
    // Move an element from the left node to the parent
    var leftNode = parentOfu.child[k - 1];
    parentOfu.elements[k - 1] =
            leftNode.elements.splice(leftNode.elements.length - 1, 1)[0];
    // Move the child link from left sibling to the node
    if (leftNode.child.length > 0)
        u.child.splice(0, 0, leftNode.child.splice(
                leftNode.child.length - 1, 1)[0]);
}

/** Perform a transfer with a right sibling */
Tree24.prototype.rightSiblingTransfer = function(k, u, parentOfu) {
    ...
    var rightNode = parentOfu.child[k + 1];
    parentOfu.elements[k] = rightNode.elements.splice(0, 1)[0];

    // Move the child link from right sibling to the node
    if (rightNode.child.length > 0)
        u.child.push(rightNode.child.splice(0, 1)[0]);
}
...
// Transfer an element from the parent to the left sibling    
    leftNode.elements.push(parentOfu.elements.splice(k - 1, 1)[0]);
    // Remove the link to the empty node
    parentOfu.child.splice(k, 1);
    // Adjust child links for non-leaf node
    if (u.child.length > 0)
        leftNode.child.push(u.child.splice(0, 1)[0]);
}
...
// Transfer an element from the parent to the right sibling
    rightNode.elements.splice(0, 0, parentOfu.elements.splice(k, 1)[0]);
    // Remove the link to the empty node
    parentOfu.child.splice(k, 1);
    // Adjust child links for non-leaf node
    if (u.child.length > 0)
        rightNode.child.splice(0, 0, u.child.splice(0, 1)[0]);
}

// Return true if the tree is empty 
Tree24.prototype.isEmpty = function() {
